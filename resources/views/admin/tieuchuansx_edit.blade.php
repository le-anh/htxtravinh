  ...
  <div class="page-section">
    <div class="card card-fluid">
      <div class="card-body">
        ...
        <form action=" {{ route('farmer.tieuchuansx.update') }} " method="post" >
          @csrf()
          @method('PUT')
          <input type="hidden" name="tieuchuansxid" value=" {{ $tieuChuanSX->id }} ">
          <fieldset>
			...
			<div class="form-group">
              ...
              <input type="text" class="form-control @error('tentieuchuansanxuat') is-invalid @enderror" id="tentieuchuansanxuat" name="tentieuchuansanxuat" value="{{old('tentieuchuansanxuat',  $tieuChuanSX->tentieuchuansanxuat)}}" placeholder="Tên tiêu chuẩn sản xuất" autofocus>
              @error('tentieuchuansanxuat')  <div class="invalid-feedback"> <i class="fa fa-exclamation-circle fa-fw"></i> {{ $message }} </div>  @enderror
            </div>
            <div class="form-group">
              <label for="mota">Mô tả </label>
              <textarea class="form-control @error('mota') is-invalid @enderror" id="mota" name="mota" rows="3" placeholder="Mô tả" autofocus>{{old('mota', $tieuChuanSX->mota)}}</textarea>
